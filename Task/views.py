...
from .serializers import TaskSerializer, TaskNameSerializer
...
class TaskNamesListAPIView(APIView):
    # permission_classes = [IsAuthenticated]
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not tasks.exists():
            return Response({"message":"no tasks availaible"}, status=status.HTTP_404_NOT_FOUND)
